...
public class TestAssertEquals {
	private static final Logger LOG = LoggerFactory.getLogger(TestAssertEquals.class);
	...
	public void test(String testQuery, String type, String returnVal) throws SQLException, IOException {
		try {
			...
			case CSV:
				...
				Assert.assertEquals(assertions.assertEquals(result.getFileName(), returnVal), true);
				break;
			case LIST:
			case MAP:
				assertions = new JSONAssertions();
				Assert.assertEquals(assertions.assertEquals(result.getList().toString(), returnVal), true);
				break;
			default:
				break;
			}
