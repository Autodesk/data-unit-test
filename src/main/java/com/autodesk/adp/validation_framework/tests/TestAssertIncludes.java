...
public class TestAssertIncludes {
	private static final Logger LOG = LoggerFactory.getLogger(TestAssertIncludes.class);
	...
	public void test(String testQuery, String type, String returnVal) throws SQLException, IOException {
		try {
			...
			case CSV:
				...
				Assert.assertEquals(assertions.assertIncludes(result.getFileName(), returnVal), true);
				break;
			case LIST:
			case MAP:
				assertions = new JSONAssertions();
				Assert.assertEquals(assertions.assertIncludes(result.getList().toString(), returnVal), true);
				break;
			default:
				break;
			}
