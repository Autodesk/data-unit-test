...
public class TestAssertOrderedEquals {
	private static final Logger LOG = LoggerFactory.getLogger(TestAssertOrderedEquals.class);
	...
	public void test(String testQuery, String type, String returnVal) throws SQLException, IOException {
		try {
			...
			case CSV:
				...
				Assert.assertEquals(assertions.assertOrderedEquals(result.getFileName(), returnVal), true);
				break;
			case LIST:
			case MAP:
				assertions = new JSONAssertions();
				Assert.assertEquals(assertions.assertOrderedEquals(result.getList().toString(), returnVal), true);
				break;
			default:
				break;
			}
